...
class PropertyPathParserTest extends AsyncFunSuite with PlatformSecrets {
  ...
  test("Sequence path") {
    assert(
        PropertyPathParser("a / b", expander) == SequencePath(
            List(
  ...
  test("Alternate path") {
    assert(
        PropertyPathParser("a|b", expander) == AlternatePath(
            List(
                PredicatePath(property = "prefix::a"),
                PredicatePath(property = "prefix::b")
            )
  ...
  test("Multi sequence path") {
    assert(
        PropertyPathParser("a/b/c/d", expander) == SequencePath(
            List(
                PredicatePath(property = "prefix::a"),
                PredicatePath(property = "prefix::b"),
  ...
  test("Multi alternate path") {
    assert(
        PropertyPathParser("a|b|c|d", expander) == AlternatePath(
            List(
                PredicatePath(property = "prefix::a"),
                PredicatePath(property = "prefix::b"),
                PredicatePath(property = "prefix::c"),
  ...
  test("Mixed path 1") {
    assert(
        PropertyPathParser("a/b|c/d|e", expander) == AlternatePath(
            List(
                SequencePath(
                    List(
                        PredicatePath(property = "prefix::a"),
  ...
  test("Mixed path 2") {
    assert(
        PropertyPathParser("a/b|c|d/e/f", expander) == AlternatePath(
            List(
                SequencePath(
                    List(
                        PredicatePath(property = "prefix::a"),
  ...
  test("Groups path 2") {
    assert(
        PropertyPathParser("a/(b|c)/((d|e)/(f/g))", expander) ==
          SequencePath(
              List(
  ...
  test("path group") {
    assert(
        PropertyPathParser("(((a/b)))", expander) == SequencePath(
            List(PredicatePath(property = "prefix::a"), PredicatePath(property = "prefix::b"))
        )
    )
  }
