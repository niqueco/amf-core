...
object ClientErrorHandlerConverter {
  ...
  def convertToClient(errorHandler: AMFErrorHandler): ClientErrorHandler =
    new ClientErrorHandler {
      override def getResults: CoreClientConverters.ClientList[AMFValidationResult] = errorHandler.getResults.asClient

      override def report(result: AMFValidationResult): Unit = errorHandler.report(result)
    }
}
