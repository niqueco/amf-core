...
object SourceInformationStage extends TransformationStep {
  ...
  override def transform(
    ...
    val additionalLocations = locationToElements.zipWithIndex.map { case ((loc, elements), i) =>
  ...
  private def buildLocationMap(
      iterator: Iterator[DomainElement],
      rootLocation: String
  ): Map[Location, List[ElementId]] = {
    iterator.foldLeft(Map.empty[String, List[String]]) { case (result, element) =>
