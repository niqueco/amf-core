...
trait OnScalarParseHook {
  def onScalarParse(node: YNode): Unit
  def onScalarParse(node: amf.core.internal.parser.domain.ScalarNode): Unit
}

object NothingOnScalarParseHook extends OnScalarParseHook {
  override def onScalarParse(node: YNode): Unit             = {}
...
class DataNodeParser private (node: YNode,
  ...
  def parse(): DataNode = {
    if (refsCounter.exceedsThreshold(node)) {
      ...
      val parsedNode = node.tag.tagType match {
        ...
        case _         => ScalarNodeParser(hook, idCounter).parse(node)
      }
      node match {
        case m: MutRef =>
  ...
  protected def parseObject(value: YMap): DataNode = {
    ...
    value.entries.map { entry =>
      ...
      val propertyNode =
        new DataNodeParser(value, refsCounter, hook, idCounter).parse().forceAdopted(node.id)
  ...
  def parse(node: YNode): DataNode = {
    node.tag.tagType match {
      ...
      case other =>
        ...
        ctx.eh
          .violation(SyamlError, parsed, None, s"Cannot parse scalar node from AST structure '$other'", node.location)
        parsed
    }
  }
        ...
        ctx.refs.find(ref => ref.origin.url == reference.text) match {
          ...
          case Some(ref) if ref.unit.isInstanceOf[EncodesModel] =>
            parseLink(reference.text, nodeLocation)
              .withLinkedDomainElement(ref.unit.asInstanceOf[EncodesModel].encodes)
          case _ =>
