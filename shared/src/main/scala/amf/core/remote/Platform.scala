...
trait Platform extends FileMediaType {
  ...
  /** Resolve remote url. */
  def fetchContent(url: String, env: BaseEnvironment)(implicit executionContext: ExecutionContext): Future[Content] =
    loaderConcat(url, env.resolvers.resourceLoaders.filter(_.accepts(url)))
