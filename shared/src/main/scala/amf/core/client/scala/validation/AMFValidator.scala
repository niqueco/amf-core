...
object AMFValidator {
  ...
  def validate(baseUnit: BaseUnit, conf: AMFGraphConfiguration): Future[AMFValidationReport] = {
    ...
    val plugins =
      conf.registry.getPluginsRegistry.validatePlugins.filter(_.applies(ValidationInfo(baseUnit, profileName))).sorted
    val constraints = computeApplicableConstraints(profileName, conf.registry.getConstraintsRules)
  ...
  private def computeApplicableConstraints(profileName: ProfileName,
