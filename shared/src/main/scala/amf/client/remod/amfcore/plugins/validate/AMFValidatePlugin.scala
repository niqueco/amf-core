...
class ValidationOptions(var profileName: ProfileName, var environment: Environment, var validations: EffectiveValidations) {
  var failFast: Boolean = true
  var forceProfile: Option[ProfileName] = None
}

trait AMFValidatePlugin extends AMFPlugin[BaseUnit] with PlatformSecrets{
  def validate(unit: BaseUnit, options: ValidationOptions)(implicit executionContext: ExecutionContext): Future[AMFValidationReport]
}
